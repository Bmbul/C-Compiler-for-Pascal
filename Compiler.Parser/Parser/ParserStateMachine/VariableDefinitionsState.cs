        ...
        public override IParsingState Handle(LexicalToken token)
        {
            ...
            while (true)
            {
                ...
                else if (nextToken.IsSpecialToken(Constants.TypesToLexem[LexicalTokensEnum.Colon])) // identifier :
                {
                    ...
                    if (nextToken.IsKeyword(Constants.TypesToLexem[LexicalTokensEnum.Integer])
                        || nextToken.IsKeyword(Constants.TypesToLexem[LexicalTokensEnum.String]))
                    {
                        Parser.AddVariables(identifiers, nextToken.Value);
                    }

                    if (!Parser.GetNextToken()
                        .IsSpecialToken(Constants.TypesToLexem[LexicalTokensEnum.Semicolon]))
                    {
